...
export default function Dashboard() {
  const { fmt, fmtShort } = useFormattedCurrency();
  ...
  analytics.forEach(({ _id, total }) => {
    ...
    if (!flowMap[key])
      flowMap[key] = { month: monthNames[_id.month], need: 0, want: 0 };
  ...
  const allocationData = [
    {
      name: "Liquid",
      value: Math.round(allocation.liquid.value / 1000),
      fill: C.blue,
    },
    {
      name: "Stocks",
      value: Math.round(allocation.stocks.value / 1000),
      fill: C.gold,
    },
    {
      name: "Mutual Funds",
      value: Math.round(allocation.mutualFunds.value / 1000),
      fill: C.purple,
    },
    {
      name: "Fixed Deposits",
      value: Math.round(allocation.fixedDeposits.value / 1000),
      fill: C.green,
    },
  ];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        ...
        <MetricCard
          ...
          sub={`Needs ${fmt(expenses.needSpend || 0)} · Wants ${fmt(expenses.wantSpend || 0)}`}
        />
        <MetricCard
          icon="🏦"
          label="Liquid Cash"
          value={fmt(allocation.liquid.value)}
          sub="Available"
        />
      </div>
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {[
              ...
              const total =
                (expenses.needSpend || 0) + (expenses.wantSpend || 0);
              return (
                <div key={r.label}>
                  <div
                    style={{
            ...
            >
              ...
              <span style={{ color: C.green }}>
                {pct(expenses.needSpend || 0, s.totalWealth * 0.01)}%
              </span>{" "}
              · Savings{" "}
              <span style={{ color: C.blue }}>
                {pct(s.totalInvested, s.totalWealth)}%
              </span>
            </div>
          </div>
        </div>
