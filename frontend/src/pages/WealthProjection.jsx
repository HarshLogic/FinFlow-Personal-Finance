...
export default function WealthProjection() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (e.response) {
        setError(
          `Error ${e.response.status}: ${e.response.data?.error || "Server error"}`,
        );
      } else if (e.request) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        <MetricCard
          ...
          subColor={C.gold}
          value={summary ? fmt(summary.finalCorpus) : "—"}
          sub={`In ${years} years`}
        />
        <MetricCard
          icon="💵"
          label="Total Invested"
          value={summary ? fmt(summary.totalInvested) : "—"}
        ...
        <MetricCard
          ...
          subColor={C.green}
          value={summary ? fmt(summary.wealthCreated) : "—"}
            ...
            <ResponsiveContainer width="100%" height={280}>
              ...
              >
                ...
                <YAxis
                  ...
                  axisLine={false}
                  tickLine={false}
                  tickFormatter={(v) => fmt(v * 100000)}
                />
                <Tooltip
                  formatter={(v, name) => {
                    if (name === "corpus")
                      return [fmt(v * 100000), "Projected Corpus"];
                    if (name === "invested")
                      return [fmt(v * 100000), "Amount Invested"];
                    return [v, name];
                  }}
                  contentStyle={{
