...
function StocksTab() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(e.response?.data?.error || "Failed to load stocks");
    } finally {
      setLoading(false);
    }
  };
  ...
  const add = async (vals) => {
    ...
    try {
      const { data } = await createStock({
        ticker: vals.ticker.toUpperCase(),
        qty: Number(vals.qty),
        avgPrice: Number(vals.avgPrice),
        cmp: Number(vals.avgPrice),
  ...
  const del = async (id) => {
    try {
      ...
      setStocks((p) => p.filter((s) => s._id !== id));
    } catch (e) {
            ...
            <tbody>
              {stocks.length === 0 ? (
                ...
                stocks.map((st) => {
                  ...
                  const plP = st.avgPrice
                    ? ((pl / (st.qty * st.avgPrice)) * 100).toFixed(2)
                    : "0.00";
                  return (
                    <tr
                      key={st._id}
                    ...
                    >
                      ...
                      <td style={{ padding: "11px 10px" }}>
                        <Pill color={C.blue}>{st.sector}</Pill>
                      </td>
                      <td style={{ padding: "11px 10px", color: C.text }}>
                        {st.qty}
                      </td>
                      <td
                        style={{
                      ...
                      <td style={{ padding: "6px 10px" }}>
                        {cmpEdit[st._id] !== undefined ? (
                          <div style={{ display: "flex", gap: 4 }}>
                            ...
                            />
                            <button
                              onClick={() => saveCMP(st._id, cmpEdit[st._id])}
                              style={{
                                background: C.gold,
                                color: "#0D0F14",
                                border: "none",
...
function MFTab() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const { data } = await getMF();
      setFunds(data);
    } catch (e) {
      setError(e.response?.data?.error || "Failed to load MFs");
    } finally {
      setLoading(false);
    }
  };
  ...
  const add = async (vals) => {
    ...
    try {
      ...
      setError(e.response?.data?.error || "Failed to add MF");
    } finally {
      setSaving(false);
    }
  };
  ...
  const mfVal = funds.reduce(
    (s, m) => s + (m.units || 0) * (m.currentNav || 0),
    0,
  );
      ...
      >
        ...
        <AddRow
          ...
          fields={[
            ...
            {
              key: "type",
              options: ["Equity", "Debt", "Hybrid", "Index", "ELSS"],
              default: "Equity",
            },
            {
              key: "invested",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {funds.length === 0 ? (
            ...
            funds.map((m) => {
              ...
              const plP = m.invested
                ? ((pl / m.invested) * 100).toFixed(2)
                : "0.00";
              return (
                <div
                  key={m._id}
                ...
                >
                  <div>
                    ...
                    <Pill color={m.type === "Equity" ? C.purple : C.blue}>
                      {m.type}
                    </Pill>
                  </div>
                  <div
                  ...
                  >
                    {[
                      ["Invested", fmt(m.invested), C.muted],
                      ["Current", fmt(cv), C.text],
                      [
                        "Return",
                        `${pl >= 0 ? "+" : ""}${plP}%`,
                        pl >= 0 ? C.green : C.red,
...
function FDTab() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const { data } = await getFDs();
      setFDs(data);
    } catch (e) {
      setError(e.response?.data?.error || "Failed to load FDs");
    } finally {
      setLoading(false);
    }
  };
  ...
  const add = async (vals) => {
    ...
    try {
      const { data } = await createFD({
        bank: vals.bank,
        principal: Number(vals.principal),
        rate: Number(vals.rate),
        tenure: Number(vals.tenure) || 12,
        startDate: vals.startDate,
      });
      setFDs((p) => [...p, data]);
    } catch (e) {
      setError(e.response?.data?.error || "Failed to add FD");
    } finally {
      setSaving(false);
    }
  };
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {fds.length === 0 ? (
            ...
            fds.map((fd) => {
              ...
              end.setMonth(end.getMonth() + fd.tenure);
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      >
                        {fd.tenure} months · {fd.rate}% p.a. · Started{" "}
                    ...
                    >
                      {[
                        ["Principal", fmt(fd.principal), C.text],
                        ["Maturity", fmt(mat), C.green],
                        ["Interest", `+${fmt(int)}`, C.gold],
                      ].map(([l, v, c]) => (
                        <div key={l} style={{ textAlign: "right" }}>
