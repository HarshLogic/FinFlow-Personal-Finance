...
// ── Currency Formatters (Hook) ──────────────────────────────────────────────
export const useFormattedCurrency = () => {
  const { currency } = useCurrency();

  const fmt = (amountInINR) => {
    if (
      amountInINR === null ||
  ...
  const fmtShort = (amountInINR) => {
    ...
    return formatCurrencyShort(amountInINR, currency);
  };
  ...
  return { fmt, fmtShort, getSymbol, getCurrencyCode };
};

// ── Formatters (Legacy - Deprecated) ──────────────────────────────────────
export const fmt = (n) => {
...
export function MetricCard({ label, value, sub, subColor, icon }) {
  return (
    ...
    >
      ...
      {sub && (
        <div style={{ fontSize: 12, color: subColor || C.muted }}>{sub}</div>
      )}
    </div>
  );
}
...
export function SectionTitle({ children }) {
  return (
    ...
    >
      <div
        style={{ width: 3, height: 18, background: C.gold, borderRadius: 4 }}
      />
      <span
        style={{
          fontSize: 15,
